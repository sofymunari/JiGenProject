...
def get_args():
    ...
    parser.add_argument("--betaRotate", type=float, default=0.2, help="percentage of data used for rotation recognition")
...
class Trainer:
    def __init__(self, args, device):
        ...
        self.args = args
        self.device = device
        self.betaRotate = args.betaRotate
        model = model_factory.get_network(args.network)(classes=args.n_classes,rotation_classes=4)
    ...
    def _do_epoch(self):
        ...
        self.model.train()
        for it, (data, class_l, rotation_l) in enumerate(self.source_loader):
            #source_loader is only data for training
            data, class_l,rotation_l = data.to(self.device), class_l.to(self.device),rotation_l.to(self.device)

            self.optimizer.zero_grad()
            
            class_logit,rotation_logit = self.model(data) #label from model
            #evaluate jigsaw mistake
            rotation_loss = criterion(rotation_logit,rotation_l)
            
            #for classification we evaluate the loss only for the not scrumbled images
            class_loss = criterion(class_logit[rotation_l == 0], class_l[rotation_l == 0])
            
            _, rotation_pred = rotation_logit.max(dim=1)
            _, cls_pred = class_logit.max(dim=1)

            loss = class_loss + self.alpha_rotation_weight * rotation_loss
            ...
            self.logger.log(it, len(self.source_loader),
                            {"Class Loss ": class_loss.item(), "Jigsaw Loss": rotation_loss.item()},
                            {"Class Accuracy ": torch.sum(cls_pred == class_l.data).item(),"Jigsaw Accuracy ": torch.sum(rotation_pred == rotation_l.data).item()},
                            data.shape[0])
            del loss, class_loss, rotation_loss, rotation_logit, class_logit
        ...
        with torch.no_grad():
            for phase, loader in self.test_loaders.items():
                ...
                class_correct,rotation_correct = self.do_test(loader)
                class_acc = float(class_correct) / total
                rotation_acc = float(rotation_correct) / total
                self.logger.log_test(phase, {"Classification Accuracy": class_acc,"Jigsaw Accuracy":rotation_acc})
                self.results[phase][self.current_epoch] = class_acc

    def do_test(self, loader):
        class_correct = 0
        rotation_correct = 0
        for it, (data, class_l, rotation_l) in enumerate(loader):
            data, class_l,rotation_l = data.to(self.device), class_l.to(self.device),rotation_l.to(self.device)
            class_logit,rotation_logit = self.model(data)
            _, rotation_pred = rotation_logit.max(dim=1)
            _, cls_pred = class_logit.max(dim=1)
            rotation_correct += torch.sum(rotation_pred == rotation_l.data)
            class_correct += torch.sum(cls_pred == class_l.data)
        return class_correct,rotation_correct
